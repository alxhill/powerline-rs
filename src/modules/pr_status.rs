...
pub struct PrStatus<S> {
    scheme: PhantomData<S>,
}

pub trait PrStatusScheme: DefaultColors {
...
impl<S: PrStatusScheme> Default for PrStatus<S> {
    fn default() -> Self {
        Self::new()
    }
}

impl<S: PrStatusScheme> PrStatus<S> {
    pub fn new() -> PrStatus<S> {
        PrStatus {
            scheme: PhantomData,
        }
    }
...
impl CheckStatus {
    ...
    fn style<S: PrStatusScheme>(self) -> (Color, Color) {
        match self {
...
struct StatusCache {
    branch: String,
    /// `None` means "looked up, but there's no PR or no checks for this branch" -
    /// cached so we don't re-query on every prompt.
    status: Option<CheckStatus>,
    fetched_at: u64,
}

impl<S: PrStatusScheme> Module for PrStatus<S> {
...
fn cache_path_for(repo_root: &Path, branch: &str) -> Option<PathBuf> {
    ...
    Some(
        base.join("powerline-rs")
            .join(format!("pr-status-{:016x}.json", hasher.finish())),
    )
}

fn read_cache(path: &Path) -> Option<StatusCache> {
...
/// hidden `refresh-pr-status` subcommand from the detached process spawned
/// above.
pub fn refresh_pr_status(branch: &str, repo_dir: &Path, cache_path: &Path) {
    let cache = StatusCache {
        branch: branch.to_string(),
        status: fetch_status(branch, repo_dir),
        fetched_at: now_secs(),
    };
...
fn fetch_status(branch: &str, repo_dir: &Path) -> Option<CheckStatus> {
    ...
    let gh: GhRollup = serde_json::from_slice(&output.stdout).ok()?;

    aggregate(&gh.status_check_rollup)
}
...
/// the module renders nothing rather than a misleading dot.
...
/// Shape of the `gh pr view --json statusCheckRollup` response we care about.
#[derive(Deserialize)]
struct GhRollup {
...
fn write_cache(path: &Path, cache: &StatusCache) {
